...
from accounts.business_features import (
    amenities_payload_for_business,
    save_business_amenities,
    save_business_working_hours,
    working_hours_payload,
)
from accounts.serializers import BusinessAmenitiesSaveSerializer
...
class PackageViewSet(viewsets.ModelViewSet):
    ...
    def amenities(self, request, pk=None):
        """Read business amenities. Writes remain for older clients; new flow uses the profile."""
        package = self.get_object()
    ...
    def working_hours(self, request, pk=None):
        """Read business working hours. Writes remain for older clients; new flow uses the profile."""
        package = self.get_object()
        business = package.business
        ...
        error = save_business_working_hours(business, request.data.get('schedule', []))
        if error:
            return Response({'error': error}, status=status.HTTP_400_BAD_REQUEST)
